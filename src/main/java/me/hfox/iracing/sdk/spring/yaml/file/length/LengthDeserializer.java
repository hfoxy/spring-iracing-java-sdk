...
public class LengthDeserializer extends JsonDeserializer<Length> {
    ...
    public Length deserialize(JsonParser p, DeserializationContext ctxt) throws IOException, JacksonException {
        ...
        if (!matcher.matches()) {
            throw new SdkJsonProcessingException("value (" + fieldValue + ") does not match pattern", p.getCurrentLocation());
        }
        ...
        LengthUnit type = LengthUnit.findByShorthand(unitValue).orElseThrow(() -> new SdkJsonProcessingException("unknown unit: " + unitValue, p.getCurrentLocation()));
