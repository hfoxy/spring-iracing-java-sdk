...
public class SpeedDeserializer extends JsonDeserializer<Speed> {

    private static final Pattern VALUE_PATTERN = Pattern.compile("([0-9\\.-]+) ([A-Za-z/]+)");

    @Override
    public Speed deserialize(JsonParser p, DeserializationContext ctxt) throws IOException, JacksonException {
        ...
        SpeedUnit type = SpeedUnit.findByShorthand(unitValue).orElseThrow(() -> new SdkJsonProcessingException("unknown unit: " + unitValue, p.getCurrentLocation()));
